...
namespace DatenClieNT
{
    public class ServiceInstaller
    {
        ...
        /// 
        public static bool InstallService(string svcPath, string svcName, string svcDescription, string svcDispName, bool autoStart, bool startNow)
        {
            ...
            try
            {
                ...
                if (sc_handle.ToInt32() != 0)
                {
                    ...
                    {
                       //Beschreibung setzen
                       //Nicht elegant aber zweckmäßig -> Direkt an die richtige Stelle in der Registry eintragen...
                       try
                       {
                          using (Microsoft.Win32.RegistryKey serviceKey = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(@"System\CurrentControlSet\Services\" + svcName, true))
                          {
                             serviceKey.SetValue("Description", svcDescription);
                          }
                       }
                       catch
                       {
