	...
	public class UAClientHelperAPI
	{
		...
		public void Connect(EndpointDescription endpointDescription, bool userAuth, string userName, string password)
		{
			try
			{
				...
				String sessionName = "IROPA DatenClientTIA Session";
		...
		public MonitoredItem AddMonitoredItem(Subscription subscription, string nodeIdString, string itemName, int samplingInterval)
		{
			...
			try
			{
				...
				//Noch nicht, erst bei ConnectComplete()
		...
		public List<object> ReadValues(List<String> nodeIdStrings)
		{
		...
		private Dictionary<string, DataValue> dataValues = new Dictionary<string, DataValue>();

		public string WriteValues(List<String> values, List<String> nodeIdStrings, string tag)
		{
			...
			foreach (String nodeIdString in nodeIdStrings)
			{
				NodeId nodeId = new NodeId(nodeIdString);
				...
				try
				{
					if (!dataValues.ContainsKey(nodeIdString))
					{
						dataValue = mSession.ReadValue(nodeId);
						dataValues[nodeIdString] = dataValue;
					}
					else
					{
						//Dadurch wird das dauernde Lesen erstmal unterdrückt
						//Nur beim ersten Mal wird gelesen
						dataValue = dataValues[nodeIdString];
					}
				}
				catch (Exception e)
				{
					return string.Format("Fehler bei WriteValues -> {0} -> {1}", e.Message, nodeIdString);
				}
				...
				try
				{
					...
					Variant variant = new Variant(Convert.ChangeType(values[nodeIdStrings.IndexOf(nodeIdString)], dataValue.Value.GetType()));
					dataValue = new DataValue(variant);
				}
				catch (Exception e123)
				{
					return string.Format("Fehler bei WriteValues nodeIdString={0} nodeIdWert={1} Type={3} | {2} -> Wenn der SPS-Datentyp z.B. int ist und der Spaltentyp in der Datenbank float. Dann findet hier kein cast statt und es kommt zu einer Exception, wenn floats mit Nachkommastellen verarbeitet werden sollen.", nodeIdString, values[nodeIdStrings.IndexOf(nodeIdString)], e123.Message, dataValue.Value.GetType());
				}
			...
			try
			{
				...
				{
					if (result[i].ToString() != "Good")
					{

						ret += Environment.NewLine;
						ret += string.Format("OPC-UA-Item schreiben fehlgeschlagen: Item={0} Fehler={1}", nodeIdStrings[i].ToString(), result[i].ToString());
					}
				}

				if (ret != "OK")
				{
					string ret1 = string.Format("UAClientHelperAPI -> {0}", ret);
					ret1 = ret1.Replace("UAClientHelperAPI -> OK", "UAClientHelperAPI -> ERR");
					throw new Exception(ret1);
				}

				watch.Stop();

				LogManager.GetSingleton().ZLog("CD34E", ELF.DEVELOPER, "{0} Items in {1} ms geschrieben ", valuesToWrite.Count, watch.PeriodMilliSeconds);
		...
		public List<string> RegisterNodeIds(List<String> nodeIdStrings)
		{
			...
			try
			{
				...
				//Maximal 5 Sekunden warten. Standard-Timeout liegt bei 6 Minuten !!!
				mSession.OperationTimeout = 5000;
